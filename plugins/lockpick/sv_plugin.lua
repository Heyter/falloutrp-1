...
--[[ UTIL ]]--
--------------
function PLUGIN:GetDoorLockpicker(door)
	door:RunOnAllDoors(function(entity, args)
		local s = entity.LockpickSession
...
function Class:StartingAction()
	...
	netstream.Start(ply, "lpStarting", true, time - ply:NetLag())
end
...
function Class:Success()
	...
	if (IsValid(door:getDoorPartner())) then
		door:RunOnAllDoors(function(e, args)
			e:Fire("unlock")
		end)
	end
	door:RunOnAllDoors(function(e, args)
...
function Class:StopCheck()
	...
	if ( time > (self.NextDistCheck or 0) ) then
		if (ply:GetLookedEntity(cfg.MaxLookDistance) ~= door) then
...
function Class:Think()
	...
	if (exceedMax) then
        if (self.AskingSuccess and self.LockAngle == cfg.UnlockMaxAngle) then
			...
			if ((SysTime() - self.HoldTime > (self.Player:NetLag()) + 0.1)) then
				local item = self.Item
