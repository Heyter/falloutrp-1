...
function PANEL:OnInit()
	...
	self.skillLbl = select( 2, self:AddInfoTab( L"lpSkill", LocalPlayer():getChar():getAttrib( "lockpick", 0 ) ) )
	self.pinsLbl = select( 2, self:AddInfoTab( L"lpPins", interface.Item:GetQuantity() ) )
	self:AddInfoTab( L"lpLockLevel" , "TO DO" )
end
...
function PANEL:DrawMain()
	...
	self.skillLbl:SetText(LocalPlayer():getChar():getAttrib( "lockpick", 0 ))
